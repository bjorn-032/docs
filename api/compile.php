...
$inFile = "$tmpDir/$entry";
$outFile = $format === 'pdf' ? "$tmpDir/output.pdf" : "$tmpDir/output-{p}.svg";
...
$cmd    = escapeshellcmd("/bin/typst") . " compile{$formatFlag} " . escapeshellarg($inFile) . " " . escapeshellarg($outFile) . $font_path_args . " 2>&1";
$output = [];
$exit   = 0;
...
if ($format === 'pdf') {
    ...
    // Typst writes output.svg (1 page) or output-1.svg, output-2.svg … (multi-page)
    $svgFiles = glob("$tmpDir/output*.svg");
    if ($exit !== 0 || !$svgFiles) {
        echo json_encode(['ok' => false, 'error' => implode("\n", $output)]);
        exit;
    }
    ...
    $pages = array_map(function($f) {
        ...
        // Fix unescaped & in SVG attribute values (e.g. URLs with query strings).
        // Replaces bare & that are not already part of a named/numeric entity reference.
